...
import {CY, EDGE_DESCRIPTION, SELECTED_NODES} from "../../redux/constants/graph/container/constants";
...
class CytoscapeDagreGraph extends Component {
        ...
        cy.nodes().forEach(function (ele) {
            if (ele.data().id === 'm') {
                tips.push(
                    {
                        'node': ele.id(),
                        'tip': makeTippy(ele, 'selected region')
                    })
            }
            tips.push(
                {
                    'node': ele.id(),
                    'tip': makeTippy(ele, ele.data().description)
                })
        });
    ...
    downloadjpg = () => {
        ...
        element.href = this.props.cy.jpg({maxWidth: 10000});
    ...
    downloadJson = () => {
        let element = document.createElement("a");
        let file = new Blob([JSON.stringify(this.props.cy.json())], {type: 'text/plain'});
        ...
        return (
            <div>
                ...
                <Grid alignItems="flex-start" container direction="row" spacing={3}>
                    ...
                    <Grid xs={6} item>
                        <Paper className={classes.paper}>
                            <SelectedNodes className="LeftFloat" selected_nodes={this.props.selected_nodes}/>
                        </Paper>
                    </Grid>
                </Grid>
            </div>
...
const connectClass = connect(mapStateToProps, actionsCreator)(CytoscapeDagreGraph);
export default (withStyles(styles)(connectClass));